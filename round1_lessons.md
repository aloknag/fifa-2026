# Round 1 (Group Stage) — Retrospective & Lessons [ARCHIVE]

> Archived 2026-06-28 after the **group stage finished**. Active knockout tracker: `Alok_FIFA_2026_Predictions.md`; active knockout lessons: `lessons.md`; full R1 match log: `round1.md`. The complete v1.0→v2.12 ledger is the appendix below.

## Final group-stage result
- **188 / 426 (44%), and we finished 2nd of 16** — Ricardo 198 (gap 7); we vaulted 4th→2nd on a **+22 final slate** (Jun 28). 11 exacts; 43/71 outcomes (61%).
- **The Jun 28 slate (+22) validated the fix to our #1 leak.** Group-final stakes were obvious (who's through / must-win / dead rubber), and **acting on them** produced **6/6 outcomes and both draw calls hitting** (ALG-AUT 3-3, COL-POR 0-0). Draw accuracy jumped 20%→33%.

## What we learned (the durable few)
1. **Outcome-first, stakes-aware.** Back must-win sides; respect the draw in content/dead-rubber/even games; X-0 a toothless eliminated opponent. This is what closed the gap.
2. **Our R1 leak was under-calling draws** (2/10 early) + margin timidity — both correctable, both improved by the end.
3. **Exacts come from a narrow profile** (elite attacker + clear stakes + opponent that plays) AND from common scorelines on clear games.
4. **Operational:** verify every pick is submitted (we lost ~5 pts in R1 to two un-submitted picks; now reload-verified).
...
# Appendix — full Round 1 calibration ledger (raw, v1.0 → v2.12)

_The evolving model behind the Round 1 predictions. (Original `lessons.md` content, preserved verbatim.)_
...
### v2.12 — The Stakes-Asymmetry / Unmotivated-Favorite Override (June 25)
Grading Jun 25 (+15, but only 2/6 outcomes) confirmed a **≥3-game pattern the v2.10 market-gate keeps mis-handling: a favorite with little or nothing to play for loses to a desperate must-win underdog far more than the moneyline implies.** The clean cases: **ECU 2-1 GER** (Germany was seed-locked as group winner regardless of result → zero incentive, fielded a partial mix; we flagged it in-row as "the KOR-RSA shadow, and worse" and STILL backed the -188 fav on the market-gate → lost); **RSA 1-0 KOR** (Jun 24 — Korea needed only a draw → passive → lost); **AUS 2-0 TUR** (Jun 14 — rotated Türkiye). Same motivation-mismatch family on the same slate: **PRY 0-0 AUS** (Australia advancing on a draw → bunkered → 0-0, our PRY-win lost) and **TUR 3-2 USA** (mutual dead rubber — eliminated Turkey played free and beat a rotated USA). The rule:
- **Motivation-asymmetry override of the market-gate.** Before backing a market favorite, check the STAKES. If the favorite is genuinely **unmotivated** — already qualified AND nothing material left to gain (seeding / top-spot locked), or visibly **rotating/resting** — AND the opponent is **must-win / desperate**, do **NOT** back the favorite at the stale line. The moneyline is slow to price full indifference + rotation. Predict the **DRAW**, or lean the desperate side if its edge is concrete. The market-gate assumes both sides are trying; when one isn't, it's invalid.
- **The trigger is objective and checkable pre-game:** (a) is the favorite already through with nothing left to improve? AND (b) is the opponent must-win/desperate? Both yes → motivation-mismatch → downgrade the favorite hard. GER-ECU was textbook on both counts and we had the read in the row — we just didn't act on it. **Acting on the flag is the whole lesson.**
...
- **Jun 26 (+11, model 4/6 outcomes): the model is sound — the 2 misses were the flagged content-favorite draws (CPV 0-0, EGY 1-1), and the elite content-favorites won (FRA 4-1, ESP 1-0). Two takeaways: (1) ACT on the content/unmotivated-favorite flag (predict the 1-1, don't just downgrade) for non-elite ones — see v2.12 extension; (2) operational — a model-correct 4-pointer (NZL-BEL) was lost to a missed app entry; confirm all picks are submitted.** **Standing (app Jun 27): Alok 4th at 169** (internal 166, ~+3 off-by-one — the NOR-FRA correction (France won 4-1) resolved the apparent +6 drift). Leader Ricardo 179 (gap 10); 3rd–5th tight at 168–170. **Group stage NOT done — Groups J/K/L play MD3 on Jun 28; R32 follows.**
...
| Jun 25 | JPN vs SWE | 2-1 → 1-1. Miss (1pt, SWE 1=1). Backed Japan to win but they only needed a draw → controlled to a 1-1. Low stakes for the favorite should have leaned draw. (→ v2.12 family) |
