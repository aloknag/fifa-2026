...
- **True coin-flips (lines ±110) with mutual desperation are genuinely 50/50 — lower confidence, maximize floor.** TUR -105 vs PAR was the slate's only miss. Both teams were desperate (TUR lost MD1, PAR lost 1-4) and the line was essentially even. An early Galarza stunner (2') and a smash-and-grab held despite Almirón's red card. When the line says "coin flip" and both teams are all-in, don't pretend you can pick the winner — use the X-0 floor for whichever side you back, and accept these are low-confidence 3pt-floor plays.
