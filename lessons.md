...
**Last updated:** 2026-06-24 — graded Jun 23 slate (**+11 pts**, no zeros but only 2.75 pts/match: POR 5-0 UZB = 3, COL 1-0 COD = 3, CRO 1-0 PAN = 4, ENG 0-0 GHA = 1 → **126/282** from 47 graded). **NEW VERSION v2.11** — the X-1 withhold misfired a 3rd time (TUN-JPN, now POR-UZB & COL-COD): every time we withheld the clean sheet against a clear/heavy favorite because the weak opponent had a "demonstrated goal," the opponent was BLANKED. COL 1-0 was the exact we listed as our own alt. **Fix: heavy favorite (≈ -200 or shorter) → take the X-0 regardless of the opponent's prior goal; reserve the X-1 withhold for moderate, genuinely-leaky favorites.** The one pick that worked (CRO 1-0 PAN, X-0, 4pts) proves it. ENG-GHA 0-0 = the recurring organized-low-block tail (ESP-CPV/ECU-CUW) — logged, no version (don't whipsaw into draw-leaning). Also added **Rule 5** to Score-Maximizing (per Alok): on a *true coin-flip* with no >50% side, predict **1-1** to maximize EV (modal score + 3pt on any draw + 1pt on narrow games) instead of a one-sided pick that goes 0 half the time. _(Prior: Jun 22 was a +21 validation slate — three exacts, 4/4 outcomes, no version.)_
...
- **The "demonstrated goal" only triggers the X-1 withhold when the favorite is MODERATE (≈ -180 or longer) AND genuinely leaks.** GER-CIV (Germany ~-160, no clean sheet in 7), NED-SWE, NZL-EGY, ALG-JOR were all moderate/balanced games where the withhold paid — keep it there.
...
- **The one Jun 23 pick that worked proves the rule:** CRO 1-0 PAN — we did NOT withhold (Panama toothless), took the X-0, and banked the 4pt floor. That is the template for heavy-favorite-vs-weak: X-0, every time.
...
When the market shows **no side above ~50%** (a genuine pickem, ±120 both ways) AND a low-scoring/tight game is expected — especially a final-round game where **both teams advance on a draw** — the highest-EV scoreline is **1-1**, not a one-sided X-0/X-1. Why: 1-1 is typically the modal *scoreline* even when one side is the modal *outcome*, and it banks the **3pt floor on ANY drawn result** (1-1 = 6, 0-0/2-2 = 3) **plus 1pt on most narrow decisive games** (the winner or loser scores exactly 1), collapsing to 0 only on a lopsided 2-0/0-2. A one-sided 1-0 instead scores **0 every time the other side wins** (~30–40% of a pickem). This is the application of "when you cannot call the outcome, pick the score that maximizes points: 6, else 4, else 3, else 1 — never bet on a 0."
...
- **Jun 23 (+11, no zeros) exposed the v2.11 leak:** we banked the floor everywhere but converted nothing to an exact because the **X-1 withhold misfired twice** — POR-UZB (2-0 was the 4pt floor) and COL-COD (1-0 was the exact). The model template that worked was CRO 1-0 PAN (took the X-0 → 4pts). **Standing (app Jun 24): Alok 3rd at 127 — leader Ricardo 129, Braiham 128; gap to 1st just 2, field tight (4th/5th at 126).** Jun 23's +11 was matched by the leaders → slipped 2nd→3rd. **The lever to retake the lead is converting the X-0 floors into exacts (v2.11) — stop softening the clean sheet on clear favorites.**
