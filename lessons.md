...
**Last updated:** 2026-06-21 — graded Jun 20 slate (**+14 pts**: GER-CIV exact 🎯, NED-SWE 4, TUN-JPN 3, ECU-CUW 1 → **86/210** from 35 graded). Outcome-first North Star again 3/4 (75%). X-0 judgment right twice (GER & NED withholds), wrong once (TUN softening). Added **v2.9** (generalize X-0 commitment to all clear favorites vs toothless sides; the "toothless-vs-threatening" test).
...
- **Draw picks: 2/8 correct (25%)** — only BEL-EGY & NED-JPN hit; KOR-CZE, CIV, FRA, ENG, GHA, MEX all missed.
...
Grading Jun 20 (+14, GER-CIV exact) showed the X-0 *judgment* is now the deciding skill — and we split 2/3 on it. The two correct calls (withholds) and one wrong call (softening) point to a single, cleaner rule than v2.8's host-only framing:
- **Generalize the X-0 commitment from "hosts" to ANY clear favorite (≈ -180 or shorter) vs a toothless/weak attack.** TUN-JPN: Japan -187 thrashed a desperate, must-win Tunisia **4-0**; we softened to 2-1 fearing they'd "commit forward," but a desperate underdog committing forward gets **picked off on the counter and blanked** — desperation ≠ they will score. The X-0 (2-0) was the 4pt floor; we banked 3. This is the same softening leak as CAN and USA. Stop fearing the weak side's goal when the favorite is clearly superior.
...
- **Jun 20 was a strong slate: +14 from 4 matches (3.5 pts/match), one exact (GER-CIV).** The X-0 judgment decided it: two correct withholds (GER, NED → 6pt + 4pt) and one bad softening (TUN 2-1 vs a toothless side that blanked → 3pt, missed the 4pt floor) → v2.9 generalizes X-0 commitment to all clear favorites vs toothless sides.
