...
_Canonical predictions tracker, maintained locally (no Google Docs). **Last updated:** 2026-06-20 — **Predicted the Jun 20 MD2 slate (Groups E & F).** Pool standing: **Alok 9th at 73** (app), leader Braiham 82 — only **9 back**, 8 players bunched 73–82; two exacts a slate closes the gap fast. New picks (next 24h): **GER 2-1 CIV, ECU 2-0 CUW, NED 2-1 SWE, JPN 2-1 TUN** — all back the favorite (MD2 regime), X-0 only where the opponent plausibly blanks (ECU-CUW). Prior: best slate yet +17 (two exacts), score **72/186** from 31 graded; added v2.8 (commit X-0 for hosts; X-1 for coin-flips). (Note: app shows 73 vs our 72 — off by one, reconciling separately.)_
...
| **June 20** | E | GER vs CIV | **2 - 1** | Pending | — | LOCKED (North Star + v2.6 MD2). Germany clear fav (**-182, bet365**), 10-match win streak, 7-1 vs CUW; but CIV is **organized**, on a 5-match win streak, beat ECU 1-0 — NOT a leaky blowout target (v2.3 ceiling doesn't apply). Germany leaks (no WC clean sheet in 7 → BTTS favored), so **don't take the X-0** — CIV scores. Back the fav win, common 2-1. **(4 PM ET, Toronto.)** Alt: 3-1 (if Ndicka out for CIV). |
| **June 20** | E | ECU vs CUW | **2 - 0** | Pending | — | LOCKED (North Star + v2.7 X-0 floor). Ecuador overwhelming fav (**-909 / 1-10**) vs Curaçao, the tournament's leakiest side (shipped 7 to GER). Both lost openers → must-win. Ecuador is a *solid-but-modest* attack (not an elite blowout machine like GER/SWE), so **don't over-lift the margin** — take the **X-0** (ECU defense strong, CUW plausibly blanks) on a common 2-0 = 4pt floor if they win. **(7 PM ET, Kansas City.)** Alt: 3-0. |
| **June 20** | F | NED vs SWE | **2 - 1** | Pending | — | LOCKED (North Star + v2.6 MD2). Netherlands fav (**-135**) but only modestly; Sweden top the group (5-1 vs TUN), unbeaten in 5 of 6. Both attack well AND leak → BTTS/Over likely, so **no X-0** — back the fav win with Sweden scoring. Not a coin-flip (NED clear fav, not ±110), so back the win, not a draw. Common 2-1. **(1 PM ET, Houston.)** Alt: 1-1 (both-leak risk). |
| **June 20** | F | TUN vs JPN | **2 - 1 (JPN)** | Pending | — | LOCKED (North Star + v2.6 MD2). Japan clear fav (**-187**), won 6 of 7, unbeaten in 8, drew NED 2-2; Tunisia 0 pts, shipped 5 to Sweden, new manager, must-win. Japan is a *technical/possession* side (not a rout machine) and Tunisia will commit forward → Tunisia can nick one, so **X-1 over X-0**. Back the fav win, common 2-1. **(11 PM ET, Monterrey.)** Alt: 2-0. |
