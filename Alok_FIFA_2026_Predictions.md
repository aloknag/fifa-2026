# Alok — FIFA World Cup 2026 Pool Predictions — KNOCKOUTS (Round of 32 →)

_Active tracker for the **knockout rounds** (Round of 32 onward). Group stage (Round 1) is archived: full match log in **`round1.md`**, retrospective + lessons in **`round1_lessons.md`**. Knockout model lives in **`lessons.md`**. Local markdown only — edit in place._

> **Carried from the group stage: 188 / 426** (71 games; app **191**, ~+3 off-by-one). **Standing: Alok 2nd of 16** — Ricardo 198 (gap **7**), Braiham 191, Salazar 190, Jesus 185. We vaulted 4th→2nd on the +22 final group slate.

> 🎯 **KNOCKOUT SCORING — same 6/4/3/1/0 as the group stage** (confirmed on Pollaya `/puntos` + by Alok; detail in `lessons.md`): **6** exact · **4** outcome + one team's goal-count · **3** outcome only · **1** wrong outcome + one goal-count · **0**. **Only knockout difference: scored on the 90/120-min result — penalties EXCLUDED, so a tie is a DRAW and predicting the shootout winner = 0.** Predict the most-likely scoreline; **1-1 for even ties (never the penalty winner)**; X-0 floor for clean-sheet favorites; don't over-inflate margins. ⚠️ **Verify every pick is submitted (reload-check).**

---

## 1. Match Tracker — Round of 32 onward
...
| Jun 30 | R32 | CIV vs NOR | **1 - 2 (NOR)** | **1 - 2 (NOR)** | **6** | **WIN (exact)** 🎯 Norway won 2-1 in normal time; CIV nicked one. The X-0 withhold paid off: predicted NOR wins with CIV scoring = the 6. _Lowest-confidence pick; alt 1-1 didn't fire._ |
| Jun 30 | R32 | FRA vs SWE | **2 - 1 (FRA)** | **3 - 0 (FRA)** | **3** | **Outcome correct; goals wrong.** France blew out Sweden 3-0; our 2-1 got the winner but neither goal count matched (FRA=2≠3, SWE=1≠0). Should have leaned X-0 — France shut Sweden out; the "withhold X-0 for Isak+Gyökeres" was too cautious vs a -400 steamroller. |
| Jun 30 | R32 | MEX vs ECU | **1 - 0 (MEX)** | **2 - 0 (MEX)** | **4** | **Outcome + one goal count.** Mexico 2-0 Ecuador; ECU=0 matched, MEX=1≠2. The X-0 read was right; we just under-estimated the margin. 4-pt floor rule worked. |
| Jul 1 | R32 | ENG vs COD | **2 - 1 (ENG)** | Pending | — | England -380 heavy fav (Bellingham/Kane/Saka); Declan Rice returns. **Withhold X-0**: Wissa (3 of COD's 4 WC goals) is in confirmed XI — a live, concrete threat. Changed 2-0 → 2-1. 🎯 aim 6 · likely 4. |
| Jul 1 | R32 | BEL vs SEN | **2 - 1 (BEL)** | Pending | — | Belgium +110 slim fav (De Bruyne/Lukaku/Trossard/Doku). **Mendy (GK) left Senegal squad with knee injury** — big defensive blow. Sarr has 3 goals but all vs Iraq; blanked vs France + Norway. Lean Belgium to win, Sarr can nick one. 🎯 aim 6 · likely 4. |
| Jul 1 | R32 | USA vs BIH | **2 - 1 (USA)** | Pending | — | USA -280 solid fav (beat PAR 4-1, AUS 2-0); BIH 3rd-place qualifier (1-1-1). **Withhold X-0**: USA only 1 clean sheet in 11 games; BTTS near coin-flip. Džeko is live. 🎯 aim 6 · likely 4. |
...
## 2. Running Scorecard (knockouts; cumulative pool total includes the group stage)
